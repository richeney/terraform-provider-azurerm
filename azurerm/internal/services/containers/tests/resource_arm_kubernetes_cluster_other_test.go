...
func TestAccAzureRMKubernetesCluster_upgradeNodePool(t *testing.T) {
	checkIfShouldRunTestsIndividually(t)
	testAccAzureRMKubernetesCluster_upgradeNodePool(t)
}
		...
		Steps: []resource.TestStep{
			{
				...
				Config: testAccAzureRMKubernetesCluster_upgradeNodePoolConfig(data, olderKubernetesVersion),
				Check: resource.ComposeTestCheckFunc(
					testCheckAzureRMKubernetesClusterExists(data.ResourceName),
					resource.TestCheckResourceAttr(data.ResourceName, "kubernetes_version", olderKubernetesVersion),
			...
			{
				// both on the newer version
				Config: testAccAzureRMKubernetesCluster_upgradeNodePoolConfig(data, currentKubernetesVersion),
				Check: resource.ComposeTestCheckFunc(
					testCheckAzureRMKubernetesClusterExists(data.ResourceName),
					resource.TestCheckResourceAttr(data.ResourceName, "kubernetes_version", currentKubernetesVersion),
...
func testAccAzureRMKubernetesCluster_upgradeNodePoolConfig(data acceptance.TestData, version string) string {
	return fmt.Sprintf(`
provider "azurerm" {
  features {}
}
...
`, data.RandomInteger, data.Locations.Primary, data.RandomInteger, data.RandomInteger, version, data.RandomInteger, version)
}
